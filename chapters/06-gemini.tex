...
AKTUALNY STAN BAZY:
${EndpointRequires.Horses(predictedEndpoint) ? `KONIE: ${konie}` : ''}
${EndpointRequires.Farriers(predictedEndpoint) ? `KOWALE: ${kowale}` : ''}
${EndpointRequires.Veterinarians(predictedEndpoint) ? `WETERYNARZE: ${weterynarze}` : ''}
${EndpointRequires.HorsesDiseases(predictedEndpoint) ? `CHOROBY: ${chorobyText}` : ''}
` : ''}
...
Efektywność tej strategii potwierdzają wyniki eksperymentalne dla końcówki \texttt{api/wydarzenia/\allowbreak{}leczenia}, przedstawione w Tabeli \ref{tab:wyniki_leczenia}. Zastosowanie wieloetapowej orkiestracji pozwoliło zwiększyć średnią precyzję z $0{,}70$ do $0{,}86$ oraz efektywność z $0{,}72$ do $0{,}95$. Wzrost tych wartości wskazuje, że model znacznie rzadziej popełniał błędy w przypisywaniu procedur medycznych do istniejących rekordów chorób, a generowane obiekty JSON były w większości przypadków poprawne i zgodne ze schematem.
...
Wieloetapowa orkiestracja & 0,86 & 0,95 \\ \hline
\end{tabular}
\end{table}
