...
\noindent
\textbf{Uwaga:} Precyzja i efektywność były liczone tylko dla poprawnie zidentyfikowanych końcówek API. W przypadku błędnego wyboru punktu końcowego miary te nie były uwzględniane w średnich (oznaczano jako NULL). Zapobiega to zniekształceniu wyników w przypadku wyboru błędnego adresu URL oraz umożliwia porównanie wyników między różnymi wersjami zapytań.
...
\begin{table}[H]
\centering
\caption{Wyniki wstępnej oceny eksperymentalnej dużych modeli językowych (podejście Zero-Shot)}
...
Analiza wyników przedstawionych w tabeli \ref{tab:llm_comparison2} wskazuje, że udoskonalone zapytanie Zero-Shot znacząco poprawiło skuteczność i precyzję generowanych odpowiedzi w porównaniu z pierwotnym podejściem (Tabela \ref{tab:llm_comparison1}). Modele rzadziej generowały nieistniejące identyfikatory i lepiej odwzorowywały strukturę danych zgodnie z wybranym schematem końcówki, co przełożyło się na poprawne wykonanie żądania w systemie REST.
...
\item \textbf{Warstwa kontekstowa (Stan bazy danych):} Zawiera wyselekcjonowane fragmenty danych, takie jak konie, kowale czy weterynarze, odpowiednie dla wybranej końcówki. Wstrzykiwanie jedynie niezbędnych informacji ogranicza ryzyko halucynacji i skłania do korzystania z istniejących rekordów.
...
\begin{itemize}
    \item \textbf{Selektywnego wstrzykiwania danych:} model operuje wyłącznie na tych fragmentach stanu bazy, które są istotne dla wybranego punktu końcowego, co ogranicza ryzyko halucynacji i błędnej interpretacji.
...
Efektywność tej strategii potwierdzają wyniki eksperymentalne dla końcówki \textit{api/wydarzenia/leczenia}, przedstawione w Tabeli \ref{tab:wyniki_leczenia}. Zastosowanie wieloetapowej orkiestracji pozwoliło zwiększyć średnią precyzję z $0,70$ do $0,86$ oraz efektywność z $0,72$ do $0,95$. Wzrost tych wartości wskazuje, że model znacznie rzadziej popełniał błędy w przypisywaniu procedur medycznych do istniejących rekordów chorób, a generowane obiekty JSON były w większości przypadków poprawne i zgodne ze schematem.

\begin{table}[h!]
\centering
\caption{Porównanie wyników dla punktu końcowego \textit{api/wydarzenia/leczenia} przed i po zastosowaniu mechanizmu orkiestracji}
...
\begin{lstlisting}[caption={Finałowa struktura zapytania dla końcówek niewymagających wieloetapowej orkiestracji}, label={wyk:final_prompt}]
ROLA:
Jesteś systemem przetwarzania zapytań użytkownika na poprawne struktury JSON zgodne z interfejsem REST. Twoim zadaniem jest wygenerowanie wyłącznie poprawnego obiektu JSON zgodnego ze schematem danych.
...
Szczególne znaczenie dla sukcesu projektu miała implementacja wieloetapowej orkiestracji zapytań, która umożliwiła rozwiązanie problemu zachowania integralności referencyjnej w bezstanowych modelach LLM. Sukces w obsłudze modułu leczenia dowodzi, że dwuetapowy proces — polegający na wstępnej rezolucji encji głównej (odpowiedniego konia w stadninie), a następnie zasileniu modelu przefiltrowaną historią chorób — jest niezbędny w przypadku relacyjnych struktur danych. Metoda ta pozwoliła na obejście ograniczeń wynikających z braku trwałej pamięci modelu, zamieniając proste odwzorowywanie tekstu w inteligentny proces syntezy informacji opartej na rzeczywistym stanie bazy danych.
