...
\noindent
\textbf{Uwaga:} Precyzja i efektywność były liczone tylko dla poprawnie zidentyfikowanych endpointów. W przypadku błędnego wyboru punktu końcowego miary te nie były uwzględniane w średnich (oznaczano jako NULL). Zapobiega to zniekształceniu wyników w przypadku błędnego wyboru endpointu oraz umożliwia porównanie wyników między różnymi wersjami zapytań.

Wszystkie testy eksperymentalne przeprowadzono w oparciu o zestaw przygotowanych endpointów REST, odpowiadających głównym obszarom funkcjonalnym systemu. Wśród obsługiwanych endpointów znalazły się:
\begin{description}
...
\item \textbf{Warstwa kontekstowa (Stan bazy danych):} Zawiera wyselekcjonowane fragmenty danych, takie jak konie, kowale czy weterynarze, odpowiednie dla wybranego endpointu. Wstrzykiwanie jedynie niezbędnych informacji ogranicza ryzyko halucynacji i skłania do korzystania z istniejących rekordów.
...
Tak przygotowany prompt zapewnia modelowi zarówno precyzyjną strukturę danych, jak i praktyczne wzorce zachowań, co zwiększa skuteczność, precyzję oraz efektywność generowanych żądań. Wykaz \ref{wyk:dynamic_prompt} przedstawia strukturę zapytania dla nowego podejścia:
...
Wprowadzenie klasyfikatora intencji oraz dynamicznej inżynierii zapytań znacząco zmniejszyło rozmiar generowanego promptu, co umożliwiło przeprowadzenie eksperymentów dotyczących optymalnej liczby przykładów \definicja{Few-Shot} dla poszczególnych końcówek REST.
...
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Endpoint} & \textbf{Liczba przykładów} & \textbf{Śr. Precyzja} & \textbf{Efektywność} \\ \hline
...
Efektywność tej strategii potwierdzają wyniki eksperymentalne dla endpointu \textit{api/wydarzenia/leczenia}, przedstawione w Tabeli \ref{tab:wyniki_leczenia}. Zastosowanie wieloetapowej orkiestracji pozwoliło zwiększyć średnią precyzję z $0,70$ do $0,86$ oraz efektywność z $0,72$ do $0,95$. Wzrost tych wartości wskazuje, że model znacznie rzadziej popełniał błędy w przypisywaniu procedur medycznych do istniejących rekordów chorób, a generowane obiekty JSON były w większości przypadków poprawne i zgodne ze schematem.

\begin{table}[h!]
\centering
\caption{Porównanie wyników dla endpointu leczeń przed i po zastosowaniu mechanizmu orkiestracji}
\label{tab:wyniki_leczenia}
\begin{tabular}{|l|c|c|}
\hline
\textbf{Podejście} & \textbf{Sr. Prezycja} & \textbf{Efektywność} \\ \hline
Pierwotne jednokrokowe & 0,70 & 0,72 \\ \hline
...
ROLA:
Jesteś systemem przetwarzania zapytań użytkownika na poprawne struktury JSON zgodne z interfejsem REST. Twoim zadaniem jest wygenerowanie wyłącznie obiektu JSON zgodnego ze schematem endpointu podstawowego.
