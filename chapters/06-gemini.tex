...
\newpage
\begin{lstlisting}[caption={Struktura pierwotnego zapytania zawierająca pełny schemat bazy danych.}, label={wyk:prompt_verbatim}]
...
\newpage
\begin{lstlisting}[caption={Struktura pierwotnego zapytania zawierająca pełny schemat bazy danych.}, label={wyk:zero-shot-prompt-engineering}]
ROLA:
...
%================ Wykaz 1 =================
\begin{lstlisting}[caption={Pierwsze zapytanie w procesie orkiestracji: ekstrakcja encji głównej (konia).}, label={wyk:leczenie_chaining_faza1}]
...
\begin{lstlisting}[caption={Finałowa struktura zapytania dla modelu z rozdzieleniem na podstawowe i złożone endpointy, z uwzględnieniem warstwy Few-Shot i selektywnego zasilenia kontekstu dla leczenia.}, label={wyk:final_prompt}]
# =========================
...
# Faza 2 i 3: Selektywne zasilenie kontekstu i generowanie końcowe
ROLA:
Jesteś systemem przetwarzania zapytań użytkownika na poprawne struktury JSON zgodne z interfejsem REST dla złożonych operacji (leczenia).
...
\begin{lstlisting}[caption={Zoptymalizowana struktura promptu zintegrowana z kontekstem bazy danych i dynamicznym schematem.}, label={wyk:aktualny_prompt}]
ROLA:
Jesteś systemem przetwarzania zapytań użytkownika na poprawne struktury JSON zgodne z interfejsem REST. Twoim zadaniem jest wygenerowanie wyłącznie poprawnego obiektu JSON zgodnego ze schematem danych.
