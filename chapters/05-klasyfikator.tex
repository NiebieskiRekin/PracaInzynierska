...
\begin{description}
    ...
    \item \textbf{Rozród:} ewidencja kryć, wyźrebień i kontroli weterynaryjnej rozrodu\\ 
    (\texttt{/api/wydarzenia/rozrody}).
    ...
    \item \textbf{Weterynaria i zdrowie:} monitorowanie procesów leczenia (\texttt{/api/wydarzenia/leczenia}), diagnozowanie chorób (\texttt{/api/wydarzenia/choroby}) 
    \item \textbf{Baza kontaktów:} zarządzanie bazą kontaktów do specjalistów\\ (\texttt{/api/kowale}, \texttt{/api/weterynarze}).
\end{description}
...
W celu optymalizacji kompromisu między obciążeniem a wariancją (\textit{bias-variance tradeoff}), w architekturze systemu  zrezygnowano z deterministycznego wyboru pojedynczej metody na rzecz podejścia zespołowego (\textit{Ensemble Learning}). Skonstruowany model finalny funkcjonuje jako komitet klasyfikatorów $\mathcal{M}$, w skład którego wchodzą algorytmy $k$-NN dla $k \in \{4, 5, 6, 8, 12\}$ oraz klasyfikator oparty na centroidach kategorii.
