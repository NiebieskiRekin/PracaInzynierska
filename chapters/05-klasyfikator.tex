...
Wdrożenie tej separacji umożliwiło zbadanie, jak klasyczne metody uczenia maszynowego operujące na wektorach semantycznych radzą sobie z branżowym słownictwem.
Problem jednak wciąż wiązał się z:
...
Dane były generowane ręcznie przez uczestników projektu, którzy na podstawie swojego doświadczenia z hodowcami koni próbowali odtworzyć ich sposób mówienia, jednocześnie zapewniając, że przykłady spełniają podstawowe wymagania systemu. Ważnym aspektem było, aby zbiór danych pokrywał różny poziom formalności przekazywania informacji stąd np. w końcówce \texttt{api/wydarzenia/\allowbreak{}rozrody} znajdują się przykłady takie jak:
...
\begin{itemize}
    ...
    \item ,,Wczoraj Tomek uczestniczył przy wyźrebieniu Giny''.
\end{itemize}
...
\begin{itemize}
    ...
    \item Dla dużych wartości $k$, następuje wzrost obciążenia (\english{bias}) i zjawisko niedouczenia (\english{underfitting}).
\end{itemize}


W celu optymalizacji kompromisu między obciążeniem a wariancją (\english{bias-variance tradeoff}), w~architekturze systemu \definicja{Moje Konie} zrezygnowano z deterministycznego wyboru pojedynczej wartości parametru $k$ na rzecz podejścia zespołowego (\english{ensemble learning}). Skonstruowany model finalny funkcjonuje jako komitet klasyfikatorów $\mathcal{M}$, w skład którego wchodzą algorytmy $k$-NN dla $k \in \{4, 5, 6, 8, 12\}$ oraz klasyfikator oparty na centroidach kategorii.

Ostateczna decyzja klasyfikacyjna podejmowana jest w procedurze głosowania większościowego (\english{majority voting}). Formalnie, wybór końcowej intencji $I_{final}$ definiuje się jako zadanie poszukiwania argumentu maksymalizującego liczbę wskazań w całym zespole:
...
W sytuacjach spornych, gdy co najmniej dwie intencje uzyskają identyczną liczbę głosów, algorytm aplikuje heurystykę rozstrzygającą (\english{tie-breaker}). Polega ona na porównaniu skumulowanej wiarygodności (\english{confidence score}), obliczanej jako suma podobieństw cosinusowych zwróconych przez te modele z komitetu, które wskazały daną klasę:
