...
Proces przesyłania danych opiera się na formacie \texttt{multipart/form-data}. Choć pliki binarne ostatecznie trafiają do zewnętrznego magazynu obiektowego (\english{bucket storage}), zachowano ten standard żądania w~celu zapewnienia spójności z~architekturą frontendu. Serwer odbiera metadane oraz pliki, przekazuje binaria do usługi składowania, a~w~bazie \textsf{PostgreSQL} zapisuje jedynie unikalny identyfikator zasobu (np. \texttt{UUID}). Takie rozwiązanie separuje logikę składowania danych od bazy, zwiększając wydajność i~skalalność systemu. % TODO: przecież od pół roku przesyłamy zdjęcia WYŁĄCZNIE na bucket, form-data pozostało bo bałem się ruszać frontu. Nie przesyłamy tu wcale JSONa, a jedynie odbieramy - done
Po pomyślnym zakończeniu transakcji, system realizuje automatyczne przekierowanie do widoku szczegółowego, co zapewnia ciągłość pracy (\english{uninterrupted workflow}) i natychmiastowe potwierdzenie zapisu danych. % TODO: czy tłumaczenie każdego konceptu na angielski jest konieczne? 
