...
W przypadku pomyślnej weryfikacji poświadczeń, serwer zwraca token JWT (ang. \textit{JSON Web Token}), który jest następnie bezpiecznie utrwalany w pamięci lokalnej przeglądarki (ang. \textit{local storage}). % TODO: tokeny w cookies
Mechanizm ten pozwala na utrzymanie stanu sesji użytkownika (ang. \textit{session persistence}) oraz bezobsługowe dołączanie tokena do nagłówków kolejnych zapytań (ang. \textit{Authorization Bearer Header}). Rozwiązanie to zapewnia bezpieczeństwo przesyłanych danych oraz pozwala na bezprzeładowywanie strony (SPA) przy jednoczesnej pełnej kontroli dostępu do chronionych zasobów inwentarzowych stadniny. % TODO: zmienił się system uwierzytelniania na Better Auth, chociaż działa na podobnej zasadzie
