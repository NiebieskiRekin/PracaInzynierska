...
W celu uzupełnienia funkcjonalności systemu oraz poprawy doświadczeń użytkownika wykorzystano szereg bibliotek pomocniczych, wśród których kluczową rolę odgrywa zestaw \textbf{Lucide React}. Jest to biblioteka udostępniająca minimalistyczne i wydajne ikony wektorowe, które pozwoliły na wprowadzenie spójnej i czytelnej identyfikacji wizualnej poszczególnych modułów aplikacji. Zastosowanie piktogramów, takich jak ikona mikrofonu dedykowana funkcji przetwarzania mowy na tekst (ang. \textit{Speech-to-Text}, STT), znacząco ułatwia użytkownikowi szybką orientację w interfejsie i skraca czas reakcji na dostępne akcje.
...
Centralny obszar pulpitu zajmuje dynamiczna siatka kart reprezentujących poszczególne zwierzęta. Dane o inwentarzu pobierane są asynchronicznie z bazy PostgreSQL, a następnie mapowane na wysokopoziomowe komponenty React. Każda karta pełni rolę kontenera informacyjnego, agregując dane tekstowe oraz multimedia. W celu poprawy interakcji z użytkownikiem zaimplementowano mechanizm podglądu zdjęć w formie pełnoekranowego okna modalnego (ang. \textit{lightbox}). Został on zrealizowany jako komponent pozycjonowany absolutnie względem okna przeglądarki. Aby zapewnić poprawną widoczność elementu nad pozostałymi częściami interfejsu, zastosowano wysoki indeks warstw (ang. \textit{z-index}), ustawiony na wartość \texttt{50}. Takie podejście gwarantuje, że podgląd zdjęcia nie zostanie przysłonięty przez inne elementy nawigacyjne czy paski boczne aplikacji, zachowując jednocześnie spójność ze stosowanym systemem projektowym (ang. \textit{design system}) oraz prostotę struktury drzewa \textsf{DOM}. % TODO: nie mamy portali, ustawiamy na sztywno z-50 - done

Nad listą inwentarza umieszczono interaktywną wyszukiwarkę, działającą w~oparciu o~stan komponentu (ang. \textit{component state}). Proces filtrowania realizowany jest po stronie klienta (ang. \textit{client-side filtering}) bezpośrednio na tablicy obiektów przechowywanej w~pamięci operacyjnej. Podejście to wybrano ze względu na charakterystykę systemu, w~którym liczba rekordów reprezentujących zwierzęta w~obrębie jednego gospodarstwa nie przekracza skali, która mogłaby negatywnie wpłynąć na wydajność przeglądarki. Dzięki wykorzystaniu wbudowanej metody \texttt{filter}, proces aktualizacji widoku odbywa się natychmiastowo po wpisaniu każdego znaku, bez konieczności oczekiwania na odpowiedź z~serwera. Eliminuje to zbędne opóźnienia sieciowe oraz redukuje obciążenie bazy danych \textsf{PostgreSQL}. Należy jednak zaznaczyć, że w~przypadku skalowania systemu do obsługi zbiorów danych liczących dziesiątki tysięcy rekordów, mechanizm ten musiałby zostać zastąpiony filtrowaniem po stronie serwera (ang. \textit{server-side filtering}) z~wykorzystaniem paginacji.
...
Proces przesyłania danych opiera się na formacie \texttt{multipart/form-data}. Choć pliki binarne ostatecznie trafiają do zewnętrznego magazynu obiektowego (ang. \textit{bucket storage}), zachowano ten standard żądania w~celu zapewnienia spójności z~architekturą frontendu. Serwer odbiera metadane oraz pliki, przekazuje binaria do usługi składowania, a~w~bazie \textsf{PostgreSQL} zapisuje jedynie unikalny identyfikator zasobu (np. \texttt{UUID}). Takie rozwiązanie separuje logikę składowania danych od bazy, zwiększając wydajność i~skalalność systemu. % TODO: przecież od pół roku przesyłamy zdjęcia WYŁĄCZNIE na bucket, form-data pozostało bo bałem się ruszać frontu. Nie przesyłamy tu wcale JSONa, a jedynie odbieramy - done
...
Największą przewagę technologiczną \definicja{Asystenta NLP} widać przy realizacji operacji masowych (ang. \textit{bulk operations}). Tradycyjne seryjne rejestrowanie zdarzeń wymaga złożonej nawigacji i manualnej selekcji zwierząt z listy, podczas gdy interfejs NLP pozwala na redukcję tego procesu do pojedynczej komendy (Rysunek \ref{fig:asystent_podkucie}).
