...
TODO - Akapit, dlaczego zero-shot nie działał. 

TODO - Akapit, przejście na Few-shot

TODO - Akapit, porównianie wyników (EDIT: w kolejnym section jest też coś o few-shot, więc tutaj chyba powinien być tylko zero-shot)
...
Wymusiło to rozpoczęcie prac nad optymalizacją instrukcji (\english{prompt engineering}). Wprowadzono dynamiczne wstrzykiwanie kontekstu (\english{context injection}) poprzez dostarczanie modelowi aktualnego stanu bazy danych (listy koni, kowali i weterynarzy), co miało na celu zbliżenie wartości precyzji i efektywności do poziomu akceptowalnego w systemie produkcyjnym.

Finalnie, w celu zapewnienia najwyższej stabilności, zastosowano mechanizm oczyszczania odpowiedzi (\english{sanitization}). Pozwala ona na odfiltrowanie zbędnych znaczników formatowania Markdown oraz komunikatów tekstowych (\english{chatter}), które modele LLM generują mimo restrykcyjnych instrukcji systemowych. 
