...
\section{Ewolucja podejścia: Od Zero-Shot do Few-Shot Prompting}

Proces opracowania metody automatycznego mapowania wypowiedzi użytkownika w języku naturalnym (ang. \textit{Natural Language Processing} – NLP) na strukturalne żądania formatu JSON (ang. \textit{JavaScript Object Notation}), zgodne z interfejsem programistycznym aplikacji (ang. \textit{Application Programming Interface} – API) typu REST (ang. \textit{Representational State Transfer}), przebiegał w sposób iteracyjny. Odzwierciedla on klasyczną ścieżkę optymalizacji systemów opartych na generatywnej sztucznej inteligencji (ang. \textit{Generative AI}). Kluczowym elementem badawczym była ewolucja podejścia: od prostych metod typu \textit{Zero-Shot} (wnioskowanie bez przykładów), aż po zaawansowaną technikę uczenia z małą liczbą przykładów (ang. \textit{Few-Shot Prompting}).

W początkowej fazie eksperymentów skupiono się na testowaniu surowych możliwości modeli wielkojęzykowych (ang. \textit{Large Language Models} – LLM) bez wykorzystania dodatkowego klasyfikatora intencji. Przyjęto wówczas założenie, że model o wystarczająco dużej liczbie parametrów będzie w stanie samodzielnie zidentyfikować cel użytkownika, dobrać odpowiedni punkt końcowy (ang. \textit{endpoint}) oraz dokonać ekstrakcji danych (ang. \textit{entity extraction}). W tym celu opracowano pierwotny wzorzec instrukcji (ang. \textit{prompt}), który zawierał jedynie ogólny opis struktury bazy danych oraz ścisłe żądanie zwrotu odpowiedzi w formacie JSON.

Pierwotna konstrukcja instrukcji charakteryzowała się znaczną objętością, co wynikało z konieczności zawarcia w jednym zapytaniu pełnej wiedzy o architekturze systemu. Na rysunku \ref{fig:pierwotny_prompt} przedstawiono strukturę tego zapytania, które składało się z kilku kluczowych sekcji:

\begin{figure}[t]
    \centering
    \includegraphics[width=0.8\textwidth]{public/prompt_structure1.png} 
    \caption{Struktura pierwotnego promptu zawierająca pełny schemat bazy danych i liczne przykłady.}
    \label{fig:pierwotny_prompt}
\end{figure}

\begin{itemize}
    \item \textbf{Definicja schematu danych:} Sekcja \texttt{\{schema\_prompt\}} zawierała kompletny opis struktur JSON dla wszystkich dostępnych punktów końcowych systemu. Powodowało to, że model musiał przetwarzać definicje wielu tabel i pól, które często były całkowicie niepowiązane z bieżącym zapytaniem użytkownika.
...
W praktyce podejście to generowało szereg problemów inżynierskich. Ze względu na wstrzykiwanie pełnej specyfikacji bazy danych (ang. \textit{database schema dumping}), prompt stawał się nadmiarowy pod względem liczby tokenów, co drastycznie zwiększało koszty operacyjne (ang. \textit{token usage}) oraz czas odpowiedzi systemu (ang. \textit{inference time}). 

Co więcej, model był zmuszony do operowania na rozbudowanym zbiorze danych wejściowych, z których znaczna część stanowiła szum informacyjny (ang. \textit{data noise}). Przykładowo, przy prostej prośbie o „dodanie nowego konia”, model musiał analizować również schematy dotyczące leczenia, kowali czy rozrodu. Prowadziło to do niepotrzebnego obciążenia mechanizmu uwagi (ang. \textit{attention mechanism}) modelu i zwiększało prawdopodobieństwo pomyłki w doborze właściwego punktu końcowego.

Dodatkowym obciążeniem było wstrzykiwanie aktualnego stanu bazy danych dla wszystkich encji jednocześnie. Dane te nie tylko konsumowały dostępne okno kontekstowe (ang. \textit{context window}), ale również prowadziły do rzadszych, lecz trudnych do zdiagnozowania błędów semantycznych, w których model próbował powiązać dane użytkownika z nieprawidłowym kontekstem pochodzącym z innej części bazy. Doświadczenia te stały się bezpośrednim impulsem do wprowadzenia dedykowanego klasyfikatora, który pozwolił na selektywne ładowanie wyłącznie tych fragmentów schematu i danych, które są krytyczne dla realizacji konkretnego żądania.

Testom poddano zróżnicowane jednostki obliczeniowe, w tym modele z rodziny GPT (ang. \textit{Generative Pre-trained Transformer}) od OpenAI (wersja 4o), a także modele otwartoźródłowe i azjatyckie, takie jak DeepSeek-V3. Wybór ten podyktowany był chęcią zbadania balansu między precyzją (ang. \textit{accuracy}) a latencją (ang. \textit{latency}), która w systemach konwersacyjnych czasu rzeczywistego ma znaczenie krytyczne.

W celu obiektywnej oceny jakości generowanych odpowiedzi, zdefiniowano trzy kluczowe miary eksperymentalne:
\begin{itemize}
    \item \textbf{Skuteczność (ang. \textit{Effectiveness}):} mierzona binarnie (0 lub 1). Wartość 1 oznacza poprawną identyfikację docelowego punktu końcowego (ang. \textit{endpoint selection}), natomiast 0 wskazuje na wybór błędnego adresu URL.
    \item \textbf{Precyzja (ang. \textit{Precision}):} mierzona w znormalizowanym zakresie od 0 do 1. Określa ona stopień zgodności wygenerowanego obiektu JSON z wzorcem oczekiwanym. Wartość 1 oznacza całkowitą zgodność wszystkich kluczy i wartości, natomiast 0 reprezentuje brak jakichkolwiek poprawnych danych w strukturze.
...
Zbiorcze wyniki wstępnej oceny modeli w podejściu \textit{Zero-Shot} przedstawiono w tabeli \ref{tab:llm_comparison}.
...
Wstępne wyniki pokazały, że podejście \textit{Zero-Shot} generuje liczne halucynacje (ang. \textit{hallucinations}), szczególnie w zakresie identyfikatorów relacyjnych. Modele często tworzyły nieistniejące numery ID lub błędnie interpretowały polskie nazewnictwo fachowe stosowane w hodowli koni, co bezpośrednio obniżało miarę precyzji i efektywności. Przykładowo, model potrafił poprawnie wybrać endpoint (skuteczność=1), lecz z powodu braku wiedzy o aktualnych rekordach w bazie, wstawiał losowe klucze obce, co skutkowało odrzuceniem żądania przez backend (efektywność=0).

Wymusiło to rozpoczęcie prac nad optymalizacją instrukcji (ang. \textit{prompt engineering}). Wprowadzono dynamiczne wstrzykiwanie kontekstu (ang. \textit{context injection}) poprzez dostarczanie modelowi aktualnego stanu bazy danych (listy koni, kowali i weterynarzy), co miało na celu zbliżenie wartości precyzji i efektywności do poziomu akceptowalnego w systemie produkcyjnym.

Finalnie, w celu zapewnienia najwyższej stabilności, zastosowano mechanizm oczyszczania odpowiedzi (ang. \textit{sanitization}). Pozwala ona na odfiltrowanie zbędnych znaczników formatowania Markdown oraz komunikatów tekstowych (ang. \textit{chatter}), które modele LLM generują mimo restrykcyjnych instrukcji systemowych. 
