...
\begin{description}
    ...
    \item[Pielęgnacja kopyt:] dokumentowanie wizyt kowala i zabiegów werkowania (\texttt{/api/wydarzenia/podkucie}).
    \item[Weterynaria i zdrowie:] monitorowanie procesów leczenia (\texttt{/api/wydarzenia/leczenia}), diagnozowanie chorób (\texttt{/api/wydarzenia/choroby}) oraz zarządzanie bazą kontaktów do specjalistów (\texttt{/api/kowale}, \texttt{/api/weterynarze}).
\end{description}
...
\begin{description}
    ...
    \item[Ekstrakcją encji:] Poza samą intencją, system musi ,,wyłowić'' z tekstu kluczowe parametry (imię konia, datę, nazwę leku), by stworzyć poprawne żądanie POST do API.
\end{description}
...
\begin{description}
	\item[Bazę wiedzy:] 60 reprezentatywnych przykładów, pełniących rolę kotwic (\english{anchors}).
...
\begin{figure}[H]
    ...
    Czerwonymi okręgami zaznaczono pięc najlepszych wyników pod względem dokładności.}
    \label{fig:knn_eval}
\end{figure}
...
W celu optymalizacji kompromisu między obciążeniem a wariancją (\textit{bias-variance tradeoff}), w~architekturze systemu \definicja{Moje Konie} zrezygnowano z deterministycznego wyboru pojedynczej wartości parametru $k$ na rzecz podejścia zespołowego (\english{ensemble learning}). Skonstruowany model finalny funkcjonuje jako komitet klasyfikatorów $k$-NN, operujących na zróżnicowanych hiperparametrach, wyselekcjonowanych na podstawie wyników eksperymentalnych (opisanych w poprzedniej sekcji).

Ostateczna decyzja klasyfikacyjna podejmowana jest w procedurze głosowania. System agreguje predykcje wszystkich klasyfikatorów składowych, przypisując użytkownikowi intencję o najwyższej liczbie wskazań. W sytuacjach spornych (remis), algorytm aplikuje heurystykę rozstrzygającą opartą na sumie podobieństw cosinusowych, co premiuje klasę o najwyższej lokalnej gęstości semantycznej względem wektora zapytania.
...
Ewaluacja końcowa modelu zintegrowanego (\english{ensemble}) wykazała wyniki przedstawione w~Tabeli~\ref{tab:final_results}. Choć są one marginalnie niższe w porównaniu do najlepszych konfiguracji pojedynczych (Tabela~\ref{tab:top_configurations}), spadek ten uznano za pomijalny koszt regularyzacji. Zwiększona stabilność predykcji daje bowiem silniejsze podstawy do prognozowania wysokiej skuteczności systemu w~środowisku produkcyjnym.
