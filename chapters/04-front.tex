...
\label{chapters:04-front}

\section{Podejście architektoniczne}
...
Fundamentem interfejsu użytkownika jest biblioteka React, która operuje na mechanizmie wirtualnego modelu
obiektowego dokumentu. Pozwala on optymalizować proces aktualizacji widoku poprzez wykonywanie operacji porównywania stanów (\english{reconciliation/diffing}), a następnie wprowadzanie jedynie niezbędnych zmian w~rzeczywistym drzewie DOM przeglądarki. Jest to rozwiązanie niezbędne w~kontekście niniejszej aplikacji, szczególnie przy dynamicznym renderowaniu wyników analizy AI~w czasie rzeczywistym. Dzięki temu interfejs \definicja{Asystenta NLP} -- modułu czatu integrującego model językowy z systemem -- pozostaje płynny i responsywny, nawet w~momencie generowania czytelnych kart podsumowujących operacji na danych, np. rejestracji wizyt weterynaryjnych czy dodania kowali. Ponadto architektura komponentowa pozwoliła na enkapsulację logiki i zarządzanie stanem aplikacji (\english{state management}) -- rozumianym jako dynamiczny zbiór danych (np. statusy ładowania czy wyniki analizy) definiujący aktualny wygląd interfejsu -- co zapewniło wysoką reużywalność kodu.

Struktura plików i katalogów wewnątrz modułu została zorganizowana w~sposób modularny, co~stanowi fundament skalowalności i łatwej konserwacji (\english{maintainability}) systemu.  Zastosowanie rozszerzeń plików \texttt{.ts} oraz \texttt{.tsx} umożliwiło kompilatorowi języka \textit{TypeScript} poprawne rozróżnienie logiki biznesowej od~komponentów zawierających składnię \textit{JSX}. Rozróżnienie to było kluczowe dla prawidłowego działania analizy typów oraz procesu budowania aplikacji. Ponadto, mechanizm statycznego typowania pozwolił na precyzyjne zdefiniowanie struktury kluczowych encji systemu, takich jak \texttt{Koń} oraz \texttt{Kowal}. Architektura aplikacji opiera się na podziale odpowiedzialności (\english{Separation of Concerns}), gdzie warstwa prezentacji jest oddzielona od logiki biznesowej i komunikacyjnej. Centralnym elementem tego układu są komponenty, czyli małe, niezależne i reużywalne jednostki interfejsu, takie jak przyciski, pola formularzy czy karty inwentarzowe. Pozwalają one na budowanie złożonych widoków przy zachowaniu wysokiej czytelności kodu źródłowego.

Wyższy poziom w~hierarchii struktury zajmują widoki (\english{pages}), reprezentujące pełnowymiarowe ekrany aplikacji, takie jak pulpit sterowania (\english{dashboard}), panel logowania czy szczegółowa karta konia. Każdy widok agreguje niezbędne komponenty i zarządza przepływem informacji pomiędzy nimi. Kluczowym aspektem inżynierskim jest mechanizm zarządzania stanem aplikacji, realizowany przy pomocy wbudowanych funkcji typu \textit{hak} (\english{hooks}). Są to specjalne funkcje biblioteki \textit{React}, które pozwalają „podpiąć się” (\english{hook into}) pod cykl życia komponentu oraz jego stan. Mechanizm ten eliminuje konieczność stosowania złożonych klas na rzecz programowania funkcyjnego. Wykorzystany w~projekcie \texttt{useState} służy do przechowywania lokalnych danych (np. treści wpisywanej w~oknie \definicja{Asystenta NLP}), wyzwalając ponowne wyrenderowanie widoku przy każdej zmianie wartości. Z~kolei \texttt{useEffect} odpowiada za obsługę efektów ubocznych i~synchronizację z~zewnętrznym interfejsem API. Jest on wywoływany automatycznie po zamontowaniu komponentu lub w~odpowiedzi na zmianę określonych zależności (np. identyfikatora konia), co umożliwia natychmiastowe odświeżenie danych bez ingerencji użytkownika. Jest to niezbędne do poprawnego funkcjonowania dynamicznych alertów medycznych, które muszą reagować na najświeższe informacje przesyłane z~serwera.
...
W celu optymalizacji przestrzeni roboczej, widok został podzielony na trzy logiczne sekcje przy użyciu elastycznych kontenerów \textit{Flexbox}. Lewa kolumna pełni funkcję panelu informacyjno-zarządczego, prezentując kluczowe parametry konia, m.in. numer przyżyciowy, numer chipa, rocznik urodzenia oraz płeć.
...
Przeniesienie obliczeń matematycznych bezpośrednio do warstwy frontendu (\english{client-side processing}) zapewnia wysoką płynność interfejsu i natychmiastową reakcję wizualną na upływający czas bez konieczności ponownego odpytywania bazy danych.
...
\begin{itemize}
    ...
    \item \textbf{Wieloetapowa klasyfikacja i inżynieria zapytań (\english{Prompt Engineering}):} Proces przetwarzania zapytania użytkownika rozpoczyna się od przesłania ciągu tekstowego do klasyfikatora intencji opisanego w rozdziale \ref{chapters:05-Klasyfikator_intencji}: \nameref{chapters:05-Klasyfikator_intencji}. Jego zadaniem jest rozpoznanie intencji (\english{intent recognition}) i dopasowanie zapytania do właściwego punktu końcowego (\english{endpoint}) interfejsu API. W kolejnym etapie, przy wykorzystaniu technik inżynierii zapytań, do modelu Gemini 2.5 Flash przesyłane jest dedykowane dla danego zadania zapytanie kontekstowe, wzbogacone o niezbędne dane systemowe (np. listy identyfikatorów koni czy dane kontaktowe specjalistów). Proces został szczegółowo przedstawiony w rozdziale \ref{chapters:06-gemini}: \nameref{chapters:06-gemini}.
...
\begin{enumerate}
    \item \textbf{Rozpoznanie specjalisty:} System kojarzy imię „Marek” z kowalem Markiem Wojtycą zapisanym w bazie.
