...
Warstwa prezentacji aplikacji \definicja{Moje Konie} stanowi kluczowy element systemu, pośredniczący między użytkownikiem a zaawansowanymi modułami przetwarzania języka naturalnego. Została ona zaprojektowana w architekturze jednostronicowej aplikacji Internetowej (\english{Single Page Application}, SPA), co zapewnia płynność interakcji zbliżoną do aplikacji natywnych poprzez eliminację konieczności przeładowywania całej strony przy nawigacji. 


 
Takie podejście architektoniczne jest kluczowe w kontekście sterowania głosowego i tekstowego -- pozwala na asynchroniczną komunikację z modelem Gemini 2.5 Flash przy jednoczesnym zachowaniu ciągłości stanu interfejsu (\english{UI state}). Przekłada się to bezpośrednio na wysoką jakość doświadczeń użytkownika (\english{User Experience}, UX), zapewniając natychmiastową reakcję wizualną na wydawane komendy.
...
Zastosowanie rozszerzeń plików \texttt{.ts} oraz \texttt{.tsx} umożliwiło kompilatorowi języka \textsf{TypeScript} poprawne rozróżnienie logiki biznesowej od~komponentów zawierających składnię \textsf{JSX}. Rozróżnienie to było kluczowe dla prawidłowego działania analizy typów oraz procesu budowania aplikacji. Ponadto, mechanizm statycznego typowania pozwolił na precyzyjne zdefiniowanie struktury kluczowych encji systemu, takich jak \texttt{Kon} oraz \texttt{Kowal}. % TODO: czy rozszerzenia nazw plików mają jakiekolwiek znaczenie? - done %TODO ładnie przenieść to
...
Głównym fundamentem interfejsu użytkownika jest biblioteka \textbf{React.js}, która została wybrana ze względu na swoją wydajność oraz deklaratywny model programowania. Kluczowym aspektem inżynierskim tej technologii jest wykorzystanie mechanizmu wirtualnego modelu dokumentu (\english{Virtual DOM}), stanowiącego lekką kopię rzeczywistego drzewa dokumentu HTML w pamięci operacyjnej. Lekkość tego rozwiązania wynika z operowania na uproszczonych obiektach JavaScript, co~pozwala unikać kosztownego przeliczania układu strony (\english{reflow}) przy każdej zmianie. React operuje na tym mechanizmie, aby optymalizować proces aktualizacji widoku poprzez wykonywanie operacji porównywania stanów (\english{reconciliation/diffing}), a następnie wprowadzanie jedynie niezbędnych zmian w rzeczywistym drzewie DOM przeglądarki. Jest to rozwiązanie niezbędne w~kontekście niniejszej aplikacji, szczególnie przy dynamicznym renderowaniu wyników analizy AI~w czasie rzeczywistym. Dzięki temu interfejs \definicja{Asystenta NLP} -- modułu czatu integrującego model językowy z systemem -- pozostaje płynny i responsywny, nawet w momencie generowania czytelnych kart podsumowujących operacji na danych, np. rejestracji wizyt weterynaryjnych czy dodania kowali. Ponadto architektura komponentowa pozwoliła na enkapsulację logiki i zarządzanie stanem aplikacji (\english{state management}) -- rozumianym jako dynamiczny zbiór danych (np. statusy ładowania czy wyniki analizy) definiujący aktualny wygląd interfejsu -- co zapewniło wysoką reużywalność kodu.
% TODO: Czym się ta lekkość objawia? - DONE
% TODO: opis asystenta NLP - DONE 
% TODO: czym jest stan w aplikacji - DONE

Jako nowoczesne narzędzie budowania (\english{build tool}) oraz serwer deweloperski wykorzystano środowisko \textbf{Vite}. Wybór ten podyktowany był koniecznością zapewnienia maksymalnej wydajności podczas procesu wytwarzania oprogramowania. Dzięki pełnemu wykorzystaniu natywnych modułów ECMAScript (\english{ES Modules}), Vite oferuje znacznie szybsze odświeżanie aplikacji w czasie rzeczywistym (\english{Hot Module Replacement}, HMR) w porównaniu do tradycyjnych narzędzi. Tradycyjne podejście opierało się na procesie kompleksowego bundlowania, czyli scalania wszystkich plików źródłowych i~ich zależności w~jeden lub kilka dużych plików wynikowych przed ich uruchomieniem w~przeglądarce. Proces ten stawał się wąskim gardłem wraz ze wzrostem rozmiaru kodu źródłowego. Vite rozwiązuje ten problem, serwując moduły bezpośrednio na żądanie przeglądarki, co drastycznie skróciło cykl deweloperski. Mechanizm ten pozwala na natychmiastową aktualizację jedynie zmienionych fragmentów kodu bez konieczności przeładowywania stanu całej aplikacji, co drastycznie skróciło cykl deweloperski. W ramach niniejszego projektu było to szczególnie istotne, gdyż umożliwiło wydajniejsze testowanie dynamicznych interakcji z modelem językowym Gemini oraz szybką iterację nad komponentami interfejsu użytkownika. Ponadto, Vite optymalizuje proces budowania wersji produkcyjnej oprogramowania poprzez zaawansowane mechanizmy usuwania nieużywanego kodu (\english{tree shaking}), co przekłada się na mniejszy rozmiar końcowych plików aplikacji i szybsze ładowanie systemu u użytkownika końcowego.
% TODO: opisać bundlowanie - done

Warstwa wizualna aplikacji opiera się na nowoczesnym frameworku \textbf{Tailwind CSS}, który wykorzystuje podejście zorientowane na klasy narzędziowe (\english{utility-first}). Rozwiązanie to umożliwiło budowanie w pełni responsywnego interfejsu użytkownika (\english{Responsive Web Design}, RWD) bezpośrednio w strukturze komponentów React, co znacząco wpłynęło na spójność kodu oraz szybkość wprowadzania modyfikacji wizualnych. Dzięki zastosowaniu wbudowanego systemu punktów przerwania (\english{breakpoints}), interfejs został precyzyjnie zoptymalizowany pod kątem pracy w specyficznych warunkach stajennych na różnorodnych urządzeniach przenośnych. Kluczowym aspektem projektowym było zapewnienie wysokiej ergonomii użytkowania, co zrealizowano poprzez implementację dużych elementów interaktywnych oraz czytelnej, wysokokontrastowej typografii, ułatwiającej obsługę aplikacji w pełnym słońcu lub przy ograniczonej precyzji ruchów. Ponadto, wykorzystanie silnika \textit{Just-in-Time} (JIT) pozwoliło na generowanie optymalnych arkuszy stylów zawierających wyłącznie faktycznie użyte klasy, co przełożyło się na minimalizację rozmiaru plików wynikowych i przyspieszenie czasu renderowania strony na urządzeniach o słabszych parametrach technicznych.

W celu uzupełnienia funkcjonalności systemu oraz poprawy doświadczeń użytkownika wykorzystano szereg bibliotek pomocniczych, wśród których kluczową rolę odgrywa zestaw \textbf{Lucide React}. Jest to biblioteka udostępniająca minimalistyczne i wydajne ikony wektorowe, które pozwoliły na wprowadzenie spójnej i czytelnej identyfikacji wizualnej poszczególnych modułów aplikacji. Zastosowanie piktogramów, takich jak ikona mikrofonu dedykowana funkcji przetwarzania mowy na tekst (\english{Speech-to-Text}, STT), znacząco ułatwia użytkownikowi szybką orientację w interfejsie i skraca czas reakcji na dostępne akcje.

Równie istotnym elementem architektury frontendu jest biblioteka \textbf{React Router}, służąca do zaawansowanego zarządzania nawigacją wewnętrzną oraz strukturą adresów URL. Dzięki jej zastosowaniu zrealizowano model nawigacji charakterystyczny dla aplikacji jednostronicowych, w~którym przejścia między poszczególnymi widokami (np. pulpitem sterowania a kartą konkretnego konia) odbywają się bez przeładowywania dokumentu HTML. Pozwoliło to na zachowanie ciągłości stanu aplikacji oraz płynności działania, co w połączeniu z asynchronicznym ładowaniem danych zapewnia wysoką responsywność całego systemu w trakcie codziennej eksploatacji.

\section{Architektura frontendu}

Struktura plików i katalogów wewnątrz modułu została zorganizowana w sposób modularny, co~stanowi fundament skalowalności i łatwej konserwacji (\english{maintainability}) systemu. Architektura aplikacji opiera się na podziale odpowiedzialności (\english{Separation of Concerns}), gdzie warstwa prezentacji jest oddzielona od logiki biznesowej i komunikacyjnej. Centralnym elementem tego układu są komponenty (\english{components}), czyli małe, niezależne i reużywalne jednostki interfejsu, takie jak przyciski, pola formularzy czy karty inwentarzowe. Pozwalają one na budowanie złożonych widoków przy zachowaniu wysokiej czytelności kodu źródłowego.



Wyższy poziom w hierarchii struktury zajmują widoki (\english{pages}), reprezentujące pełnowymiarowe ekrany aplikacji, takie jak pulpit sterowania (\english{dashboard}), panel logowania czy szczegółowa karta konia. Każdy widok agreguje niezbędne komponenty i zarządza przepływem informacji pomiędzy nimi. Kluczowym aspektem inżynierskim jest mechanizm zarządzania stanem aplikacji, realizowany przy pomocy wbudowanych funkcji typu \textit{hak} (\english{hooks}). Są to specjalne funkcje biblioteki \textsf{React}, które pozwalają „podpiąć się” (\english{hook into}) pod cykl życia komponentu oraz jego stan. Mechanizm ten eliminuje konieczność stosowania złożonych klas na rzecz programowania funkcyjnego. Wykorzystany w~projekcie \texttt{useState} służy do przechowywania lokalnych danych (np. treści wpisywanej w~oknie \definicja{Asystenta NLP}), wyzwalając ponowne wyrenderowanie widoku przy każdej zmianie wartości. Z~kolei \texttt{useEffect} odpowiada za obsługę efektów ubocznych i~synchronizację z~zewnętrznym interfejsem \textsf{API}. Jest on wywoływany automatycznie po zamontowaniu komponentu lub w~odpowiedzi na zmianę określonych zależności (np. identyfikatora konia), co umożliwia natychmiastowe odświeżenie danych bez ingerencji użytkownika. Jest to niezbędne do poprawnego funkcjonowania dynamicznych alertów medycznych, które muszą reagować na najświeższe informacje przesyłane z~serwera.


Istotnym elementem struktury jest mechanizm komunikacji z~warstwą serwerową, oparty na technologii \textbf{Hono RPC} (\english{Remote Procedure Call}). Zamiast tradycyjnego katalogu usług z~ręcznie definiowanymi zapytaniami, wykorzystano współdzielony klient RPC, który zapewnia pełną synchronizację typów pomiędzy frontendem a~backendem w~czasie rzeczywistym. Dzięki temu każda zmiana w~punktach końcowych API lub strukturze danych na serwerze jest natychmiast wykrywana przez kompilator \textsf{TypeScript} w~warstwie wizualnej, co eliminuje ryzyko błędów typu \textit{runtime}. Integralną częścią tego procesu jest biblioteka \textbf{Zod}, służąca do deklaratywnej walidacji schematów danych. Pozwala ona na ścisłe zdefiniowanie kontraktów danych dla encji takich jak \texttt{Konie}, \texttt{Kowale} czy \texttt{Wydarzenia}. Tak zaprojektowana architektura gwarantuje, że dane przesyłane pomiędzy interfejsem użytkownika a~bazą danych \textsf{PostgreSQL} są zawsze zgodne ze zdefiniowanym schematem, co zapewnia najwyższy poziom integralności systemu i~znacząco ułatwia proces refaktoryzacji kodu.
...
Ekran logowania (Rysunek \ref{fig:logowanie}) stanowi punkt wejściowy do systemu, dlatego został zaprojektowany z naciskiem na minimalizm oraz intuicyjność. Warstwa wizualna oparta na gradientach w~odcieniach zieleni buduje spójną identyfikację wizualną (\english{visual identity}) z tematyką hodowlaną aplikacji.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{public/logowanie.png} 
    \caption{Widok panelu logowania do aplikacji \definicja{Moje Konie}.}
    \label{fig:logowanie}
\end{figure}
...
Pasek nawigacyjny (\english{Navbar}) został zaprojektowany jako element o pozycji stałej (\english{fixed}), co oznacza, że jest on trwale przytwierdzony do górnej krawędzi okna przeglądarki. W przeciwieństwie do standardowych elementów interfejsu, pasek ten nie jest przewijany wraz z pozostałą zawartością strony, co zapewnia użytkownikowi natychmiastowy dostęp do kluczowych modułów systemu (Konie, Weterynarze, Kowale, Wydarzenia, Asystent) w dowolnym momencie pracy. Jest to rozwiązanie szczególnie istotne przy zarządzaniu rozbudowanymi zbiorami danych, gdzie manualne powracanie na początek długiej listy inwentarza byłoby nieefektywne i negatywnie wpływało na ergonomię pracy.

W ramach komponentu zastosowano technikę segmentacji kolorystycznej w celu obniżenia obciążenia poznawczego użytkownika. Poszczególne kategorie otrzymały unikalne barwy — kolor zielony dla modułu koni oraz niebieski dla sekcji weterynaryjnej — co pozwala na błyskawiczną orientację w strukturze systemu bez konieczności analizy etykiet tekstowych. Szczególne znaczenie przypisano przyciskowi dedykowanemu \definicja{Asystentowi NLP}, który wyróżniono kolorem fioletowym, sygnalizując jego zaawansowany charakter oparty na mechanizmach sztucznej inteligencji. Stała widoczność asystenta zachęca użytkownika do korzystania z interfejsu konwersacyjnego jako alternatywy dla tradycyjnej nawigacji.

\subsection{Zarządzanie listą inwentarza}

Centralny obszar pulpitu zajmuje dynamiczna siatka kart reprezentujących poszczególne zwierzęta. Dane o inwentarzu pobierane są asynchronicznie z bazy PostgreSQL, a następnie mapowane na wysokopoziomowe komponenty React. Każda karta pełni rolę kontenera informacyjnego, agregując dane tekstowe oraz multimedia. W celu poprawy interakcji z użytkownikiem zaimplementowano mechanizm podglądu zdjęć w formie pełnoekranowego okna modalnego (\english{lightbox}). Został on zrealizowany jako komponent pozycjonowany absolutnie względem okna przeglądarki. Aby zapewnić poprawną widoczność elementu nad pozostałymi częściami interfejsu, zastosowano wysoki indeks warstw (\english{z-index}), ustawiony na wartość \texttt{50}. Takie podejście gwarantuje, że podgląd zdjęcia nie zostanie przysłonięty przez inne elementy nawigacyjne czy paski boczne aplikacji, zachowując jednocześnie spójność ze stosowanym systemem projektowym (\english{design system}) oraz prostotę struktury drzewa \textsf{DOM}. % TODO: nie mamy portali, ustawiamy na sztywno z-50 - done

Nad listą inwentarza umieszczono interaktywną wyszukiwarkę, działającą w~oparciu o~stan komponentu (\english{component state}). Proces filtrowania realizowany jest po stronie klienta (\english{client-side filtering}) bezpośrednio na tablicy obiektów przechowywanej w~pamięci operacyjnej. Podejście to wybrano ze względu na charakterystykę systemu, w~którym liczba rekordów reprezentujących zwierzęta w~obrębie jednego gospodarstwa nie przekracza skali, która mogłaby negatywnie wpłynąć na wydajność przeglądarki. Dzięki wykorzystaniu wbudowanej metody \texttt{filter}, proces aktualizacji widoku odbywa się natychmiastowo po wpisaniu każdego znaku, bez konieczności oczekiwania na odpowiedź z~serwera. Eliminuje to zbędne opóźnienia sieciowe oraz redukuje obciążenie bazy danych \textsf{PostgreSQL}. Należy jednak zaznaczyć, że w~przypadku skalowania systemu do obsługi zbiorów danych liczących dziesiątki tysięcy rekordów, mechanizm ten musiałby zostać zastąpiony filtrowaniem po stronie serwera (\english{server-side filtering}) z~wykorzystaniem stronicowania.
% TODO: Czyli, w wolnym tłumaczeniu, pobieracie całą tabelę z bazy danych do klienta i tam robicie całe filtrowanie, nieważne że stadnina jest w Chinach i w związku z tym ma ok. 170 milionów koni? - done
% TODO: co to znaczy wysokowydajnej metody filter??? - done

\subsection{Adaptacja do urządzeń mobilnych}

Kluczowym aspektem inżynierskim było zapewnienie pełnej funkcjonalności aplikacji w środowisku mobilnym, co wynika ze specyfiki pracy w terenie. W procesie adaptacji interfejsu (Rysunek \ref{fig:mobile_view}) zastosowano strategię \textit{Mobile First}, wykorzystując system siatki frameworka Tailwind CSS. Dzięki modyfikatorom responsywnym, układ automatycznie transformuje się z wielokolumnowej siatki desktopowej do pojedynczej kolumny na smartfonach, zachowując optymalną wielkość celów dotykowych (\english{touch targets}). 

Ograniczona przestrzeń ekranowa wymusiła zastosowanie menu typu hamburger oraz reorganizację elementów sterujących. Przyciski akcji zostały zoptymalizowane pod kątem obsługi kciukiem (\english{thumb-friendly design}) poprzez rozciągnięcie ich do pełnej szerokości kontenera. Tak zaprojektowana warstwa prezentacji nie stanowi jedynie uproszczonej wersji systemu, lecz jest pełnowartościowym narzędziem mobilnym, ściśle zintegrowanym z systemowym interfejsem mikrofonu, co umożliwia wydajną obsługę komend NLP bezpośrednio przy zwierzętach.
...
Istotnym aspektem inżynierskim jest wdrożony mechanizm bezpiecznego usuwania zasobów. Przycisk „Usuń konia” został celowo wyposażony w dodatkowy krok potwierdzający, realizowany poprzez komponent typu modal, co ma na celu zapobieganie przypadkowej utracie danych przez użytkownika. Co więcej, w logice systemowej zastosowano technikę łagodnego usuwania (\english{soft delete}) opisaną w podrozdziale \ref{sec:pozafunkcjonalne}: \nameref{sec:pozafunkcjonalne}. Takie rozwiązanie pozwala na wykluczenie obiektu z bieżącego wyświetlania w interfejsie użytkownika, przy jednoczesnym zachowaniu pełnej spójności relacyjnej bazy danych oraz integralności historycznej systemu.

\paragraph{Multimedialna kolumna środkowa i generowanie raportów}

Środkowa sekcja koncentruje się na dokumentacji wizualnej zwierzęcia. Zaimplementowano w niej galerię zdjęć z mechanizmem dynamicznego podglądu, gdzie każde zdjęcie po kliknięciu aktywuje tryb pełnoekranowy (\english{modal view}), umożliwiając szczegółową inspekcję stanu konia. System zarządza plikami w sposób asynchroniczny, pozwalając na dodawanie nowych fotografii oraz ich trwałe usuwanie z serwera plików.
...
Prawa strona interfejsu zawiera moduł „Aktywne zdarzenia”, w którym wdrożono autorski system \textbf{warunkowego renderowania kolorystycznego}, wspomagany przez symbole graficzne. Logika aplikacji w czasie rzeczywistym oblicza różnicę między datą bieżącą a terminem zabiegu, przypisując mu odpowiedni status wizualny. Zgodnie z~zasadami projektowania uniwersalnego, komunikacja statusu nie opiera się wyłącznie na barwach, ale również na unikalnych piktogramach, co zapewnia dostępność interfejsu dla osób z~zaburzeniami rozpoznawania barw (np. daltonizmem):
\begin{itemize}
    \item \textbf{Kolor czerwony oraz symbol „\texttimes” (Status krytyczny):} sygnalizuje zdarzenia przeterminowane, wymagające bezzwłocznej interwencji.
    \item \textbf{Kolor pomarańczowy oraz symbol „--” (Status ostrzegawczy):} aktywowany automatycznie, gdy do terminu pozostało 7 dni lub mniej, co pozwala na logistyczne zaplanowanie wizyty specjalisty.
    \item \textbf{Kolor zielony oraz symbol „\checkmark” (Status poprawny):} potwierdza aktualność zabiegów profilaktycznych.
\end{itemize}

% TODO: A jak taki dobór kolorów ma się do projektowania uniwersalnego? Daltonizm jest w miarę popularnym problemem - dodanie emotek - done

Przeniesienie obliczeń matematycznych bezpośrednio do warstwy frontendu (\english{client-side processing}) zapewnia wysoką płynność interfejsu i natychmiastową reakcję wizualną na upływający czas bez konieczności ponownego odpytywania bazy danych. % TODO: ja bym to raczej przemilczał, przecież i tak pobieramy wszystkie dane przy każdym odświeżeniu strony, a logika biznesowa powinna być w backendzie
...
Kluczowym elementem interakcji z relacyjną bazą danych stadniny są formularze systemowe, które umożliwiają zarówno wprowadzanie nowych osobników do inwentarza, jak i aktualizację informacji o już istniejących koniach. Interfejs wprowadzania danych (Rysunek \ref{fig:add_horse_form}) został zaprojektowany w formie centralnie umieszczonej karty, co zapewnia spójność wizualną z ekranem logowania i buduje jednolite doświadczenie użytkownika. Zastosowanie pionowego układu pól (\english{stacked layout}) znacząco poprawia czytelność interfejsu oraz ułatwia wypełnianie danych na urządzeniach mobilnych, co jest priorytetem w specyficznych warunkach pracy terenowej.
...
W celu optymalizacji struktury kodu oraz ułatwienia późniejszej konserwacji systemu, zaimplementowano mechanizm komponentu współdzielonego. Dzięki tej architekturze, moduł edycji danych wykorzystuje tę samą strukturę i logikę co formularz dodawania. Różnica polega na automatycznej inicjalizacji pól (\english{pre-filling}) aktualnymi danymi pobranymi asynchronicznie z~bazy danych dla konkretnego rekordu, co minimalizuje redundancję kodu źródłowego i ułatwia wprowadzanie zmian w walidacji czy układzie graficznym. % TODO: wcale tak nie jest, są 2 osobne strony

\paragraph{Techniczna obsługa danych wejściowych i multimediów}

Implementacja formularzy opiera się na zaawansowanej koncepcji komponentów kontrolowanych (\english{controlled components}), co gwarantuje pełną kontrolę nad stanem aplikacji w każdej milisekundzie interakcji. Wykorzystanie funkcji typu \textbf{hak} — \texttt{useState} — pozwala na przechowywanie danych w formie \textbf{stanu aplikacji} (\english{state}), czyli dynamicznego obiektu rezydującego w pamięci komponentu. Każda interakcja użytkownika aktualizuje konkretne właściwości (klucze) tego obiektu, odpowiadające polom w~bazie danych. Wstępną walidację zapewniają natywne typy pól \textsf{HTML5}, co ogranicza przesyłanie błędnych struktur do interfejsu \textsf{API}.
% TODO: hook - done
% TODO: Czym są odpowiednie klucze, czym się różnią od nieodpowiednich i czy stan to aby nie pojęcie krawieckie? - done
...
Proces przesyłania danych opiera się na formacie \texttt{multipart/form-data}. Choć pliki binarne ostatecznie trafiają do zewnętrznego magazynu obiektowego, zachowano ten standard żądania w~celu zapewnienia spójności z formularzem. Serwer odbiera metadane pliku i~zapisuje w~bazie danych jedynie unikalny identyfikator zasobu (\texttt{UUID}). Klient przekazuje pełny plik zdjęcia do usługi magazynu obiektowego poprzez użycie mechanizmu \textit{podpisanego odnośnika} opisanego w rozdziale \ref{sec:gcs}: \nameref{sec:gcs}. Takie rozwiązanie separuje logikę składowania danych, których reprezentacja w relacyjnej bazie danych jest utrudniona, zwiększając wydajność i~skalalność systemu. Po pomyślnym zakończeniu transakcji, system realizuje automatyczne przekierowanie do widoku szczegółowego, co zapewnia ciągłość pracy i natychmiastowe potwierdzenie zapisu danych. % TODO: ta form-data jest trochę bez kontekstu, w większości api jest json, a tylko do dodawania konia form-data
...
Moduł zarządzania bazą kowali (Rysunki od \ref{fig:lista_kowali} do \ref{fig:edytuj_kowala}) został zaimplementowany w sposób analogiczny do modułu weterynarzy, co stanowi praktyczną realizację zasady wysokiej reużywalności komponentów warstwy prezentacji. Z perspektywy projektowania interfejsów, zastosowanie identycznego układu tabelarycznego oraz spójnego schematu kolorystycznego dla obu grup specjalistów znacząco zmniejsza obciążenie poznawcze użytkownika (\english{cognitive load}). Dzięki temu hodowca, operując w różnych sekcjach systemu, wykorzystuje te same wyuczone schematy poruszania się po interfejsie (\english{mental models}), co podnosi ogólną efektywność obsługi aplikacji.
...
Zastosowanie opisanych mechanizmów w praktyce hodowlanej znacznie redukuje tzw. koszt interakcji (\english{interaction cost}). Poprzez zastąpienie wielokrotnego wypełniania identycznych formularzy sekwencją selekcji grupowej i pojedynczego zatwierdzenia, system minimalizuje czas potrzebny na administrację inwentarza. Przekłada się to bezpośrednio na realne oszczędności czasowe kadry stajennej oraz eliminację błędów wynikających z monotonii wprowadzania danych, co podnosi ogólny poziom bezpieczeństwa i jakości zarządzania stadniną.

\section{Innowacyjny interfejs \definicja{Asystenta NLP}}

Sercem aplikacji \definicja{Moje Konie} jest \definicja{Asystenta NLP} (Natural Language Processing), który rewolucjonizuje paradygmat zarządzania danymi w środowisku hodowlanym. Rozwiązanie to zastępuje tradycyjne, czasochłonne wypełnianie formularzy interfejsem konwersacyjnym, umożliwiając komunikację z systemem za pomocą języka naturalnego. Użytkownik może wydawać polecenia zarówno w formie tekstowej, jak i głosowej, co znacząco obniża barierę wejścia i przyspiesza proces cyfryzacji zdarzeń stajennych.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{public/asystent_pusty.png}
    \caption{Interfejs czatu z \definicja{Asystenta NLP} przed wprowadzeniem zapytania.}
    \label{fig:asystent_pusty}
\end{figure}

Interfejs \definicja{Asystenta NLP} (Rysunek \ref{fig:asystent_pusty}) charakteryzuje się minimalistycznym wzornictwem, co ma na celu skupienie uwagi użytkownika na interakcji głosowo-tekstowej. Pod warstwą wizualną moduł ten realizuje złożony, wieloetapowy proces przetwarzania informacji.

\begin{itemize}
    \item \textbf{Przetwarzanie mowy na tekst (\english{Speech-to-Text}, STT):} Dzięki integracji interfejsu z systemowym modułem mikrofonu (oznaczonym żółtą ikoną w polu wprowadzania), użytkownik ma możliwość głosowego dyktowania komend. Rozwiązanie to jest kluczowe z punktu widzenia ergonomii pracy w warunkach stajennych, gdzie bezpośrednia obsługa manualna urządzenia jest często utrudniona ze względu na konieczność jednoczesnego sprawowania opieki nad zwierzęciem.
    \item \textbf{Wieloetapowa klasyfikacja i inżynieria zapytań (\english{Prompt Engineering}):} Proces przetwarzania zapytania użytkownika rozpoczyna się od przesłania ciągu tekstowego do klasyfikatora. Jego zadaniem jest rozpoznanie intencji (\english{intent recognition}) i dopasowanie zapytania do właściwego punktu końcowego (\english{endpoint}) interfejsu API. W kolejnym etapie, przy wykorzystaniu technik inżynierii zapytań, do modelu Gemini 2.5 Flash przesyłane jest dedykowane dla danego zadania zapytanie kontekstowy, wzbogacone o niezbędne dane systemowe (np. listy identyfikatorów koni czy dane kontaktowe specjalistów). Szczegółowy opis mechanizmu decyzyjnego został opisany w rozdziale \ref{}:\nameref{}, a struktura zapytań została zawarta w rozdziale \ref{}: \nameref{} poświęconym implementacji \definicja{Asystenta NLP}. % TODO: Uzupełnij odniesienia po napisaniu rozdziału
    \item \textbf{Realizacja operacji i zwrotna informacja wizualna:} Po przetworzeniu instrukcji przez model językowy, system wykonuje zaprogramowaną logikę biznesową bezpośrednio na bazie danych (np. utrwalenie nowego rekordu lub aktualizacja statusu). Wynik operacji przesyłany jest asynchronicznie do warstwy prezentacji, gdzie komponent \definicja{Asystenta NLP} generuje ustrukturyzowaną odpowiedź. Łączy ona naturalny komunikat tekstowy z elementami graficznymi (\english{UI cards}), co pozwala użytkownikowi na natychmiastową weryfikację poprawności wykonanego zadania bez konieczności opuszczania widoku czatu.
\end{itemize}

Asystent wykracza poza ramy prostego bota informacyjnego, pełniąc funkcję aktywnego agenta z uprawnieniami do modyfikacji stanów bazy danych. Na Rysunku \ref{fig:asystent_kowal} przedstawiono proces automatycznego dodawania specjalisty do bazy. Dzięki mechanizmowi rozpoznawania encji nazwanych (\english{Named Entity Recognition}, NER), system samodzielnie identyfikuje imię, nazwisko, zawód oraz numer telefonu wewnątrz swobodnej wypowiedzi, eliminując potrzebę ręcznego przełączania się pomiędzy widokami i wypełniania pól formularza.
...
Największą przewagę technologiczną \definicja{Asystenta NLP} widać przy realizacji operacji masowych (\english{bulk operations}). Tradycyjne seryjne rejestrowanie zdarzeń wymaga złożonej nawigacji i manualnej selekcji zwierząt z listy, podczas gdy interfejs NLP pozwala na redukcję tego procesu do pojedynczej komendy (Rysunek \ref{fig:asystent_podkucie}).
...
Po przetworzeniu, \definicja{Asystent NLP} zwraca listę koni, których dane zostały zaktualizowane. Jest to potężne narzędzie, które przekształca aplikację z pasywnej bazy danych w aktywnego partnera w zarządzaniu stadniną.
