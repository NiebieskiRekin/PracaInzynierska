...
Fundamentem interfejsu użytkownika jest biblioteka React, która operuje na mechanizmie wirtualnego modelu dokumentu. Pozwala on optymalizować proces aktualizacji widoku poprzez wykonywanie operacji porównywania stanów (\english{reconciliation/diffing}), a następnie wprowadzanie jedynie niezbędnych zmian w~rzeczywistym drzewie DOM przeglądarki. Jest to rozwiązanie niezbędne w~kontekście niniejszej aplikacji, szczególnie przy dynamicznym renderowaniu wyników analizy AI~w czasie rzeczywistym. Dzięki temu interfejs \definicja{Asystenta NLP} -- modułu czatu integrującego model językowy z systemem -- pozostaje płynny i responsywny, nawet w~momencie generowania czytelnych kart podsumowujących operacji na danych, np. rejestracji wizyt weterynaryjnych czy dodania kowali. Ponadto architektura komponentowa pozwoliła na enkapsulację logiki i zarządzanie stanem aplikacji (\english{state management}) -- rozumianym jako dynamiczny zbiór danych (np. statusy ładowania czy wyniki analizy) definiujący aktualny wygląd interfejsu -- co zapewniło wysoką reużywalność kodu.

Struktura plików i katalogów wewnątrz modułu została zorganizowana w~sposób modularny, co~stanowi fundament skalowalności i łatwej konserwacji (\english{maintainability}) systemu.  Zastosowanie rozszerzeń plików \texttt{.ts} oraz \texttt{.tsx} umożliwiło kompilatorowi języka \textit{TypeScript} poprawne rozróżnienie logiki biznesowej od~komponentów zawierających składnię \textit{JSX}. Rozróżnienie to było kluczowe dla prawidłowego działania analizy typów oraz procesu budowania aplikacji. Ponadto, mechanizm statycznego typowania pozwolił na precyzyjne zdefiniowanie struktury kluczowych encji systemu, takich jak \texttt{Kon} oraz \texttt{Kowal}. Architektura aplikacji opiera się na podziale odpowiedzialności (\english{Separation of Concerns}), gdzie warstwa prezentacji jest oddzielona od logiki biznesowej i komunikacyjnej. Centralnym elementem tego układu są komponenty, czyli małe, niezależne i reużywalne jednostki interfejsu, takie jak przyciski, pola formularzy czy karty inwentarzowe. Pozwalają one na budowanie złożonych widoków przy zachowaniu wysokiej czytelności kodu źródłowego.
